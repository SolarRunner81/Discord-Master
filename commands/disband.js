...
module.exports = {
	name: 'disband',
	description: 'Disband the party',
	async execute(message, Party) {
		const rowCount = await Party.destroy({where: {},truncate: true});
		if (!rowCount) return message.reply('No party members in the party!');

		return message.channel.send('Whole Party disbanded');
	}
}